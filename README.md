...
当前 `admin-service` 已提供 `/admin/dashboard` 首页聚合接口，展示 MQ、本地消息、日志、通知、Excel 和文件中心的核心数量，并会暴露默认管理员密码是否已修改的安全状态；也提供 `/admin/traces/{traceId}` 聚合接口，可串起操作/API 日志、MQ 失败消息和本地消息；前端“链路追踪”页面会展示汇总计数、时间线和三类明细。系统管理变更会强制写入 `sys_operation_log`，登录成功/失败会写入 `sys_login_log` 并可在“登录日志”页面查询。
