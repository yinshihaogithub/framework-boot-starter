...
| 分包 | 职责 |
| --- | --- |
| `dashboard` | 后台首页统计、模块状态、默认密码安全状态、MQ/本地消息/通知/Excel/文件中心概览 |
...
| 文件统计 | GET | `/admin/files/stats` |
| 文件列表 | GET | `/admin/files` |
| 上传文件 | POST | `/admin/files` |
| 下载文件 | GET | `/admin/files/{id}/download` |
...
文件中心会调用 `framework-file` 的存储抽象保存真实文件，并把文件 key、原始文件名、大小、
Content-Type、业务类型和上传人写入 `framework_file_record`，后台支持下载和软删除。
