...
| `excel` | Excel 导入导出任务、错误明细、示例导出 |
| `codegen` | MySQL 表结构读取、MVC 三层工程代码预览生成 |
...
| 创建示例导出任务 | POST | `/admin/excel/tasks/demo-export` |
| 创建示例失败任务 | POST | `/admin/excel/tasks/demo-failure` |
| Excel 错误明细 | GET | `/admin/excel/tasks/{taskId}/errors` |
| 代码生成表列表 | GET | `/admin/codegen/tables` |
| 代码生成字段列表 | GET | `/admin/codegen/tables/{tableName}/columns` |
| 代码生成预览 | POST | `/admin/codegen/preview` |
| 日志列表 | GET | `/admin/logs` |
...
导入错误明细统一落 MySQL。通知发送测试会调用 `framework-notify` 的通道门面，Excel 示例导出
会调用 `framework-excel` 真实生成 xlsx 字节，并把任务结果写入 `framework_excel_task`。

代码生成中心从当前 MySQL 库读取元数据，生成 Entity、Mapper、Service、Controller、Vue 页面和菜单 SQL 的预览。
生成代码遵循 `Controller -> Service -> Mapper` 三层结构，Mapper 使用 MyBatis 注解方式声明 SQL；
Java 文件按 `{module}.entity`、`{module}.mapper`、`{module}.service`、`{module}.controller` 分包输出；
写操作只保证本地事务，由 Service 使用 `TransactionTemplate` 手动开启事务。第一版只返回预览内容，
不直接写入工程文件；真正落盘应在确认覆盖策略、包路径和权限后再开启。
