...
public class LoginSecurityService {
    ...
    public void recordLoginFailure(String username) {
        ...
        if (count != null && count >= maxFailCount) {
            ...
            log.warn("[登录锁定] username={}, 失败{}次, 锁定{}分钟", username, count, lockDurationMinutes);
            throw new AuthException(ResultCode.ACCOUNT_LOCKED,
                    "密码错误次数过多，账号已被锁定 " + lockDurationMinutes + " 分钟");
        }
        log.warn("[登录失败] username={}, 当前失败次数={}/{}", username, count, maxFailCount);
    }
    ...
    public void clearLoginFailure(String username) {
        redis.delete(LOGIN_FAIL_PREFIX + username);
    }
    ...
    public long getFailCount(String username) {
        String count = redis.opsForValue().get(LOGIN_FAIL_PREFIX + username);
    ...
    public void unlock(String username) {
        redis.delete(ACCOUNT_LOCK_PREFIX + username);
        redis.delete(LOGIN_FAIL_PREFIX + username);
        log.info("[账号解锁] username={}", username);
    }
}
